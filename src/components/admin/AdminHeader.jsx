...
const navigate = useNavigate();
  ...
  const handleLogoutConfirm = () => {
    ...
    navigate("/auth/login"); // Abhi ke liye login page par navigate karega
  };  
  const dropdownRef = useRef(null);

  // Resize par sidebar aur profile dropdown dono ko reset karna
  useEffect(() => {
    const handleResize = () => {
  ...
  // Outside click handle karne ke liye (Dropdown close karne ke liye)
  useEffect(() => {
    ...
    <div className="flex flex-col w-full sticky top-0 z-50 bg-white shadow-sm">
      <div className="w-[98%]">
        <div className="flex justify-between items-center h-16 px-4 md:px-6">
          ...
          <div className="flex justify-center items-center gap-x-3 relative" ref={dropdownRef}>
            {/* Notification Bell */}
            ...
            <div className="hidden sm:flex items-center gap-x-3">
              ...
              <button onClick={() => setIsLogoutModalOpen(true)} className="group flex justify-center text-red-500 text-xs items-center gap-2 hover:bg-red-400 hover:text-white cursor-pointer p-2 rounded-xl transition-all duration-300">
                <svg 
            ...
            {showProfileDropdown && (
              <div className="absolute right-0 top-14 bg-white border border-gray-100 shadow-xl rounded-2xl p-4 w-52 flex flex-col gap-y-3 sm:hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                <div className="flex flex-col border-b border-gray-50 pb-2.5">
                  <span className="font-semibold text-sm text-gray-900">Imran Khan</span>
                  <span className="text-xs text-gray-400 truncate mt-0.5">imranKhan@gmail.com</span>
                </div>
                
                <button onClick={() => setIsLogoutModalOpen(true)} className="group w-full flex items-center justify-between text-red-500 text-xs font-semibold p-2.5 rounded-xl hover:bg-red-50 transition-colors cursor-pointer">
