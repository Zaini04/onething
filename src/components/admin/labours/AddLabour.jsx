...
import { labourValidation } from "../../../validations/LabourValidation";
...
const AddLabour = () => {
  ...
  const editData = location.state?.labourData;
  ...
  const labourMutation = useMutation({
    mutationFn: async (payload) => {
      ...
      if (isEditMode) {
        return Axios.put(`/labour/${editData._id}`, payload);
      }
      return Axios.post('/labour/add_labour', payload);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["labours"] });
      queryClient.invalidateQueries({ queryKey: ["laboursDropdown"] });
      setIsSubmitting(false);
      formik.resetForm();

      setProfileImage(null);

      toast.success(isEditMode ? "Labour updated successfully!" : "Labour created successfully!");
      navigate("/app/labours");
    },
    onError: (err) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      labourMutation.mutate(submissionPayload);
    },
  });
  ...
  return (
    <div className="w-full md:w-[80%] lg:w-[85%] xl:w-[87%]  p-4 md:pl-8 bg-[#F9FAFB]  rounded-2xl">
      <div className="flex justify-between items-center text-gray-900 mb-6 tracking-tight">
        <h2 className="text-lg font-medium">{isEditMode ? "Edit Labour" : "Add Labour"}</h2>
        <button
          type="button"
          disabled={labourMutation.isLoading}
          onClick={() => navigate("/app/labours")}
      ...
      >
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 mb-8">
          ...
          <div>
            <div className="flex gap-3 mb-1">
              <label className={`cursor-pointer bg-black text-white px-4 py-2 rounded-xl text-xs font-medium hover:bg-gray-800 transition shadow-sm ${labourMutation.isLoading ? "opacity-50 pointer-events-none" : ""}`}>
                Upload new
                <input
                  type="file"
                  accept="image/*"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-7">
          ...
          <FormInput
            ...
            placeholder="Enter labour's father or husband name"
            formik={formik}
          />
          ...
          <FormInput
            ...
            id="whatsAppNumber"
            placeholder="Enter labour whatsApp number"
            formik={formik}
          />
          <FormInput
            label="Email"
          ...
          <FormInput
            ...
            id="address"
            placeholder="Enter labour address or flat number"
            formik={formik}
          />
          <SearchSelect
            label="City"
